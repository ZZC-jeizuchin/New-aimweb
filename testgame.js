...
const MODE_CONFIG = {
    ...
    mode_tracking: {
        ...
        health: 1500,
        speedMin: 5, speedMax: 7,
        burstMultMin: 1.3, burstMultMax: 1.5,
        burstDecay: 5,
        dirChangeMin: 500, dirChangeMax: 1600,
        scorePerHit: 240, scorePenalty: 40
    },
    mode_pos: {
...
const state = {
    ...
    // ----- 新增開鏡相關 -----
    isADS: false,
    adsSensitivity: 0.645,        // 根據你的測量：開鏡/腰射 = 2.0/3.1 ≈ 0.645
    adsFovMultiplier: 0.8,        // 1.25倍倍率 -> 視野縮小為 1/1.25 = 0.8
};
...
const AimTelemetryManager = {
    ...
    drawTrend(history) {
        ...
        ctx.fillRect(padding.left + 55, 8, 12, 3);
        ctx.fillStyle = '#aaa';
        ctx.fillText('命中率', padding.left + 73, 9);
    },
    ...
    drawRadar(sectorData, globalTTK, isInsufficient = false) {
        ...
        angles.forEach((ang, i) => {
            ...
            if (ttk > worstTTK && stat.hits > 0) { worstTTK = ttk;
                worstSector = i; }
    ...
    applyProfile(profileKey) {
        ...
        if (isCustom) {
            ...
            if (config.useRatio) {
                ...
                DOMElements.inputs.aspectRatioSelect.value = '16:9';
            }
        }
...
const Game = {
    ...
    spawnTrackingTarget() {
        ...
        const speedProgress = Math.random();
...
function setupEventListeners() {
    ...
    DOMElements.buttons.clearScores.forEach(btn => btn.addEventListener('click', e => { e.stopPropagation();
...
function handleMouseMove(event) {
    ...
    let sens = state.sensitivity;
    if (state.isADS) {
        sens *= state.adsSensitivity;
    }
    euler.y -= event.movementX * 0.002 * sens;
    euler.x -= event.movementY * 0.002 * sens;
...
function buildEnvironment() {
    ...
    if (state.sceneTheme === 'space') {
        ...
        floor.position.z = extendedCenterZ;
        envGroup.add(floor);
        ...
        for (let i = 0; i < starCount; i++) {
            ...
            const y = radius * Math.cos(phi) + ROOM_SIZE.height;
        ...
        ceiling.position.z = extendedCenterZ;
...
function updateTracking(now, dt) {
    ...
    if (t.position.x > 15) { t.position.x = 15;
        t.userData.targetDir = -1;
        t.userData.currentBurst = t.userData.burstMult; }
    if (t.position.x < -15) { t.position.x = -15;
        t.userData.targetDir = 1;
    ...
    if (intersects.length > 0) {
        ...
        if (t.userData.health <= 0) {
            ...
            state.reactionTimes.push(lifespan);
